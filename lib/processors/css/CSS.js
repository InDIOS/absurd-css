...
var toCSS = function (rules, options, indent) {
	...
	for (var selector in rules) {
		...
		if (~selector.indexOf("____raw")) {
			var rawSel = /(____raw_[0-9])/.exec(selector)[0];
...
// combining selectors
var combineSelectors = function (rules, preventCombining, keepCamelCase) {
	...
	for (var i = 0; i < map.length; i++) {
		if (map[i].combine === true && map[i].selector !== false) {
			for (var j = i + 1; j < map.length; j++) {
				var propi = keepCamelCase ? map[i].prop : transformUppercase(map[i].prop);
				var propj = keepCamelCase ? map[j].prop : transformUppercase(map[j].prop);
...
var replaceDefined = function (css, options) {
	if (options && options.api && options.api.getStorage().__defined) {
		...
		var delimiters = options.api.delimiters || ['{%', '%}'];
		for (var prop in storage) {
			var re = new RegExp(delimiters[0] + '( )?' + prop + '( )?' + delimiters[1], 'g');
...
module.exports = function () {
	var processor = function (rules, callback, options) {
		...
		if (options && options.api && options.api.dynamicCSS) {
			css = require('../../helpers/TemplateEngine')(css, options);
		}
		// Minification
		if (options.minify) {
			css = minimize(css);
