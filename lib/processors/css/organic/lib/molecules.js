...
function colorRgba(value) {
	switch (value.length) {
		case 2:
			return { color: 'rgba(' + value[0] + ',' + value[0] + ',' + value[0] + ',' + value[1] + ')' };
		case 3:
			return { color: 'rgba(' + value[0] + ',' + value[1] + ',' + value[2] + ',1)' };
		case 4:
			return { color: 'rgba(' + value[0] + ',' + value[1] + ',' + value[2] + ',' + value[3] + ')' };
		default:
			return { color: 'rgba(' + value[0] + ',' + value[0] + ',' + value[0] + ',1)' };
	}
}

function colorHsla(value) {
	switch (value.length) {
		case 2:
			return { color: 'hsla(' + value[0] + ',' + (value[1] >= 1 ? (value[1] + '%,' + value[1] + '%,1)') : ('100%,100%,' + value[1] + ')')) };
		case 3:
			var col = {};	
			if (value[2] > 1 || value[2] < 0) {
				col.color = 'hsla(' + value[0] + ',' + value[1] + '%,' + value[2] + '%,1)';
			} else {
				col.color = 'hsla(' + value[0] + ',' + value[1] + '%,' + value[1] + '%,' + value[2] + ')';
			}	
			return col;
		case 4:
			return { color: 'hsla(' + value[0] + ',' + value[1] + '%,' + value[2] + '%,' + value[3] + ')' };
		default:
			return { color: 'hsla(' + value[0] + ',100%,100%,1)' };
	}
}

["col", "colh", "colr"].forEach(function (type) {
	exports[type] = (function (color) {
		return function (value) {
			value = value || (type === 'colh' ? '359' : '255');
			var val = value.split(' ');
			if (val[0].charAt(0) === '#') {
				return { color: val[0] };
			} else {
				var key = color.split('col')[1];
				switch (key) {
					case 'h':
						return colorHsla(val);
					case 'r':
