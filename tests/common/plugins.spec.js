describe("Extending", function() {
		...
		Absurd(function(api) {
			...
			api.plugin('brand-font-size', function(api, type) {
				switch(type) {
					...
					case "medium": return { 'font-size': '22px'};
					case "big": return { 'font-size': '32px'};
	...
	it("should create plugin which depends on other plugin", function(done) {
		Absurd(function(api) {
			...
			api.plugin('brand-font-size', function(api, type) {
				switch(type) {
					case "small": return { 'font-size': '12.5px', 'brand-color': ''};
					case "medium": return { 'font-size': '22px', 'brand-color': ''};
					case "big": return { 'font-size': '32px', 'brand-color': ''};
					default: return { 'font-size': '12px', 'brand-color': ''};
				}
			});
